...
public class MaxHeap<T extends Comparable<T>> implements Heap<T> {
	...
	public MaxHeap(ArrayList<T> list,HeapType heapType){
		if(list.isEmpty()){
			new MaxHeap<T>();
		}else{
		this.heapList = list;
	...
	private void sink(int k, int N) {
        ...
        while (2*k <= N) {
            int j = 2*k;
            if (j < N && less(j,j+1)) j++; // to find out which child is greater
            if (!less(k,j)) break;
	...
	private boolean isSorted(){
        int N = heapList.size();
		for (int i = 1; i < N; i++)
            if (less(heapList.get(i-1),heapList.get(i))) return false;
        return true;
    }
	...
	// A MaxHeap sorts elements in ascending order
	public void sort() {
		if(!isSorted()){
			heapify();
		}
	...
	// Gives the sorted elements in descending order
	...
	public Heap<T> makeMaxHeap(ArrayList<T> list){
		Heap<T> buildHeap = new MaxHeap<T>(list,HeapType.MAX);
		return buildHeap;		
	}
    ...
    public static void main(String[] args) {
        ...
        MaxHeap<String> MaxHeap = new MaxHeap<String>(a,Heap.HeapType.MAX);
        MaxHeap.printHeap();
        MaxHeap.insert("A");
        MaxHeap.insert("D");
        MaxHeap.insert("Z");
